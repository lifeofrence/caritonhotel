...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
  try {
    ...
    <html>
      ...
      if ($clientEmailSent) {
        header("Location: booking.html?message=" . urlencode("Booking successful! Confirmation email has been sent to $email"));
      } else {
        header("Location: booking.html?message=" . urlencode("Booking received! We will contact you shortly to confirm."));
      }
      exit;
    } catch (Exception $e) {
      ...
      if ($clientEmailSent) {
        header("Location: booking.html?message=" . urlencode("Booking successful! Confirmation email has been sent to $email (Admin notification failed)."));
      } else {
        header("Location: booking.html?message=" . urlencode("Booking received! We will contact you shortly to confirm (Email notifications failed)."));
      }
      exit;
    }
  } catch (Exception $e) {
    error_log("Booking Error: " . $e->getMessage());
    header("Location: booking.html?error=" . urlencode("Booking failed: " . $e->getMessage()));
    exit;
  }
}
